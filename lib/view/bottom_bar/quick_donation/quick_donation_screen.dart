  ...
  return Padding(
    ...
    child: Obx(()=>Row(
      children: [
        ...
        if (viewModel.goNext.value&&!viewModel.showPaymentMethod.value) 10.horizontalSpace,
        if(!viewModel.showPaymentMethod.value)Expanded(
            ...
            onPressed: () {
              if (viewModel.goNext.value) {
                if (userBox.isNotEmpty) {
                  ...
                  if (user.roles.contains("Admin") || user.roles.contains("Orgainizations")) {
                ...
                if (viewModel.selectedProjectsList.isEmpty) {
                  Utils.showGlobalSnackBar(message: "pleaseSelectProject".tr);
                } else {
                  viewModel.goNext.value = true;
                  final cartViewModel = Get.find<CartViewModel>();
                  cartViewModel.cartCount.value = viewModel.selectedProjectsList.length;
                }
              }
            },
          ),
...
Widget _selectedProjects() {
  ...
  return Expanded(
    child: Obx(() => ListView(
      ...
      children: [
        ...List.generate(viewModel.selectedProjectsList.value.length, (index) {
          ProjectElements project = viewModel.selectedProjectsList[index];
          return KeyboardActions(
            autoScroll: false,
            config: Utils.buildConfig(Get.context!, [KeyboardActionsItem(displayArrows: false,focusNode: project.focusNode)]),
            child: Column(
