...
class FaqsScreen extends GetView<CMSFaqViewModel> {
  ...
  Widget _buildAddFAQBtn() {
    if (!controller.canAdd) {
      return SizedBox.shrink();
    }
    return addElevatedButton(
        onPressed: () => controller.addNewFAQ(), text: "addNewFAQ");
  }
    ...
    return CupertinoSearchField(
      ...
      onChanged: (val) {
        if (val.trim().isEmpty) {
          ...
          controller.fetchFaqs(clear: true);
        }
      },
      onClear: () {
        ...
        if (controller.canExport)
          Expanded(
            child: expandedChip(
              ...
              onPressed: () => controller.exportFAQs(),
            ),
          ),
  ...
  Obx _buildListView() {
    return Obx(() => ListView.separated(
          ...
          itemBuilder: (_, int index) => faqItem(controller.faqs[index]),
        ));
  }

  Container faqItem(FaQs faq) {
    String category = "";
    FaqCategory? cat = controller.categories
        .firstWhereOrNull((cat) => cat.categoryId == faq.categoryId);
    if (cat != null) {
      category = Utils.isArabic ? cat.titleArabic : cat.title;
    }
    String status = Utils.statusIntoString(faq.requestStatus);
    List<DashboardData> projectDetails = [
      DashboardData(
          title: "title",
          value: Utils.isArabic ? faq.questionArabic : faq.question),
      DashboardData(title: "category", value: category),
      DashboardData(
          title: "createdDate",
          value: faq.createdDate != null
              ? Utils.dateFormat1.format(faq.createdDate!.toLocal())
              : ""),
      DashboardData(
          title: "publishDate",
          value: faq.publishDate != null
              ? Utils.dateFormat1.format(faq.publishDate!.toLocal())
              : ""),
    ];
    return Container(
      ...
      child: Column(
        ...
        children: [
          listViewHeaderPopUpMenu(
              status: status,
              onSelected: (value) => controller.onMenuSelected(value, faq),
            ...
            child: Row(
              ...
              children: [
                ...
                CupertinoSwitchWidget(
                  value: faq.isActive,
                  onChanged: controller.canEdit && faq.requestStatus == 2
                      ? (val) {
                          controller.enableDisable(faq);
                        }
                      : null,
                ),
              ],
