...
class PublicDocumentsScreen extends GetView<PublicDocumentsViewModel> {
  const PublicDocumentsScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.white,
      appBar: myAppBar(title: "publicDocuments"),
      body: _buildBody(),
    );
  }
  ...
  Obx _buildListView() {
    return Obx(() => ListView.separated(
          ...
          itemCount: controller.publicDocuments.length,
          separatorBuilder: (_, int index) => 16.verticalSpace,
          itemBuilder: (_, int index) =>
              documentsItem(controller.publicDocuments[index]),
        ));
  }
  ...
  Widget _buildUploadDocumentBtn() {
    if (!controller.canAdd) {
      return SizedBox.shrink();
    }
    return addElevatedButton(
      onPressed: () => controller.uploadDocDialog(),
      text: "uploadDocument",
  ...
  Container documentsItem(PublicDocuments document) {
    List<DashboardData> projectDetails = [
      ...
      DashboardData(
          title: "updatedBy",
          value: Utils.isArabic ? document.userNameAr : document.userName),
      DashboardData(
          title: "uploadedDate",
            ...
            child: Padding(
              ...
              child: popupMenuButton(
                  ...
                  menuItems: [
                    ...
                    popupMenuItem(
                        label: "copyLink",
                        icon: AppResources.copyIcon,
                        textStyle:
                            AppTextStyle.secondaryPrimaryBlack14spTextStyle2),
                    popupMenuItem(
                        label: "download",
  ...
  Padding _buildSwitchBtn(PublicDocuments document) {
    return Padding(
